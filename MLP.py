...
class MLP:
	...
	def initialise(self):
		"""Initialise the weights of the network at random"""
		nin,nhid,nout = self.structure
		self.w1 = np.random.randn(nin,nhid)
		self.b1 = np.random.randn(1,nhid)
		self.w2 = np.random.randn(nhid,nout)
		self.b2 = np.random.randn(1,nout)
	...
	def gradient(self,weights,x,t):
		...
		return self.backpropagate(x,delta_out) + self.alpha*weights
	...
	def error(self,weights,x,t):
		...
		self.unpack(weights)
		y = self.forward(x)
		return self.error_fn(y,t) + 0.5*self.alpha*np.sum(np.square(weights))
